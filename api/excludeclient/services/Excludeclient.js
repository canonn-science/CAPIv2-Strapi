...
module.exports = {
  ...
  fetchAll: (params) => {
    ...
    return Excludeclient.query(function(qb) {
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Excludeclient.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Excludeclient._attributes)
      .filter(attribute => attribute !== Excludeclient.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Excludeclient._attributes[attribute].type));
