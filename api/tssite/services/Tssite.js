...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const withRelated = populate || Tssite.associations
      .filter(ast => ast.autoPopulate !== false)
      .map(ast => ast.alias);

    const filters = convertRestQueryParams(params);

    return Tssite.query(buildQuery({ model: Tssite, filters }))
  ...
  count: (params) => {
    ...
    return Tssite.query(buildQuery({ model: Tssite, filters: _.pick(filters, 'where') })).count();
  },
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = await Tssite.forge(params).save(data);
