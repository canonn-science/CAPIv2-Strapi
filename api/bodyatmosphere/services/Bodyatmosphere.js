...
module.exports = {
  ...
  remove: async (params) => {
    ...
    await Bodyatmosphere.updateRelations(params);

    return Bodyatmosphere.forge(params).destroy();
  },

  /**
   * Promise to search a/an bodyatmosphere.
   *
   * @return {Promise}
   */
  ...
  search: async (params) => {
    ...
    const associations = Bodyatmosphere.associations.map(x => x.alias);
    const searchText = Object.keys(Bodyatmosphere._attributes)
      .filter(attribute => attribute !== Bodyatmosphere.primaryKey && !associations.includes(attribute))
      .filter(attribute => ['string', 'text'].includes(Bodyatmosphere._attributes[attribute].type));

    const searchNoText = Object.keys(Bodyatmosphere._attributes)
      .filter(attribute => attribute !== Bodyatmosphere.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Bodyatmosphere._attributes[attribute].type));

    const searchInt = Object.keys(Bodyatmosphere._attributes)
      .filter(attribute => attribute !== Bodyatmosphere.primaryKey && !associations.includes(attribute))
      .filter(attribute => ['integer', 'decimal', 'float'].includes(Bodyatmosphere._attributes[attribute].type));

    const searchBool = Object.keys(Bodyatmosphere._attributes)
      .filter(attribute => attribute !== Bodyatmosphere.primaryKey && !associations.includes(attribute))
      .filter(attribute => ['boolean'].includes(Bodyatmosphere._attributes[attribute].type));

    const query = (params._q || '').replace(/[^a-zA-Z0-9.-\s]+/g, '');

    return Bodyatmosphere.query(qb => {
