...
module.exports = {
  ...
  fetchAll: (params) => {
    // Get model hook
    const hook = strapi.hook[Gytype.orm];
    ...
    return Gytype.query(function(qb) {
      // Generate match stage.
      hook.load().generateMatchStage(qb)(Gytype, filters);
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Gytype.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Gytype._attributes)
      .filter(attribute => attribute !== Gytype.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Gytype._attributes[attribute].type));
