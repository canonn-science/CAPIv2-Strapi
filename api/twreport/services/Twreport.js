...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const withRelated = populate || Twreport.associations
      .filter(ast => ast.autoPopulate !== false)
      .map(ast => ast.alias);

    const filters = convertRestQueryParams(params);

    return Twreport.query(buildQuery({ model: Twreport, filters }))
  ...
  count: (params) => {
    ...
    return Twreport.query(buildQuery({ model: Twreport, filters: _.pick(filters, 'where') })).count();
  },
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = await Twreport.forge(params).save(data);
