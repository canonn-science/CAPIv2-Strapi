...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const withRelated = populate || Btreport.associations
      .filter(ast => ast.autoPopulate !== false)
      .map(ast => ast.alias);

    const filters = convertRestQueryParams(params);

    return Btreport.query(buildQuery({ model: Btreport, filters }))
  ...
  count: (params) => {
    ...
    return Btreport.query(buildQuery({ model: Btreport, filters: _.pick(filters, 'where') })).count();
  },
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = await Btreport.forge(params).save(data);
