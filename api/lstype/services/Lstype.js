...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const withRelated = populate || Lstype.associations
      .filter(ast => ast.autoPopulate !== false)
      .map(ast => ast.alias);

    const filters = convertRestQueryParams(params);

    return Lstype.query(buildQuery({ model: Lstype, filters }))
  ...
  count: (params) => {
    ...
    return Lstype.query(buildQuery({ model: Lstype, filters: _.pick(filters, 'where') })).count();
  },
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = await Lstype.forge(params).save(data);
