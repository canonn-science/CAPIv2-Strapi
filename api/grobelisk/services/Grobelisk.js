...
module.exports = {
  ...
  fetchAll: (params) => {
    // Get model hook
    const hook = strapi.hook[Grobelisk.orm];
    ...
    return Grobelisk.query(function(qb) {
      ...
      hook.load().generateMatchStage(qb)(Grobelisk, filters);
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Grobelisk.forge(params).save(data);
  ...
  remove: async (params) => {
    ...
    await Grobelisk.updateRelations(params);

    return Grobelisk.forge(params).destroy();
  },

  /**
   * Promise to search a/an grobelisk.
   *
   * @return {Promise}
   */
  ...
  search: async (params) => {
    ...
    const associations = Grobelisk.associations.map(x => x.alias);
    const searchText = Object.keys(Grobelisk._attributes)
      .filter(attribute => attribute !== Grobelisk.primaryKey && !associations.includes(attribute))
      .filter(attribute => ['string', 'text'].includes(Grobelisk._attributes[attribute].type));

    const searchNoText = Object.keys(Grobelisk._attributes)
      .filter(attribute => attribute !== Grobelisk.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Grobelisk._attributes[attribute].type));

    const searchInt = Object.keys(Grobelisk._attributes)
      .filter(attribute => attribute !== Grobelisk.primaryKey && !associations.includes(attribute))
      .filter(attribute => ['integer', 'decimal', 'float'].includes(Grobelisk._attributes[attribute].type));

    const searchBool = Object.keys(Grobelisk._attributes)
      .filter(attribute => attribute !== Grobelisk.primaryKey && !associations.includes(attribute))
      .filter(attribute => ['boolean'].includes(Grobelisk._attributes[attribute].type));

    const query = (params._q || '').replace(/[^a-zA-Z0-9.-\s]+/g, '');

    return Grobelisk.query(qb => {
