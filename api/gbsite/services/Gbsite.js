...
module.exports = {
  ...
  fetchAll: (params) => {
    // Get model hook
    const hook = strapi.hook[Gbsite.orm];
    ...
    return Gbsite.query(function(qb) {
      // Generate match stage.
      hook.load().generateMatchStage(qb)(Gbsite, filters);
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Gbsite.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Gbsite._attributes)
      .filter(attribute => attribute !== Gbsite.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Gbsite._attributes[attribute].type));
