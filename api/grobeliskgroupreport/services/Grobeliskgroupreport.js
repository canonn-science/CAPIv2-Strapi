...
module.exports = {
  ...
  fetchAll: (params) => {
    // Get model hook
    const hook = strapi.hook[Grobeliskgroupreport.orm];
    ...
    return Grobeliskgroupreport.query(function(qb) {
      ...
      hook.load().generateMatchStage(qb)(Grobeliskgroupreport, filters);
  ...
  remove: async (params) => {
    ...
    await Grobeliskgroupreport.updateRelations(params);

    return Grobeliskgroupreport.forge(params).destroy();
  },

  /**
   * Promise to search a/an grobeliskgroupreport.
   *
   * @return {Promise}
   */
  ...
  search: async (params) => {
    ...
    const associations = Grobeliskgroupreport.associations.map(x => x.alias);
    const searchText = Object.keys(Grobeliskgroupreport._attributes)
      .filter(attribute => attribute !== Grobeliskgroupreport.primaryKey && !associations.includes(attribute))
      .filter(attribute => ['string', 'text'].includes(Grobeliskgroupreport._attributes[attribute].type));

    const searchNoText = Object.keys(Grobeliskgroupreport._attributes)
      .filter(attribute => attribute !== Grobeliskgroupreport.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Grobeliskgroupreport._attributes[attribute].type));

    const searchInt = Object.keys(Grobeliskgroupreport._attributes)
      .filter(attribute => attribute !== Grobeliskgroupreport.primaryKey && !associations.includes(attribute))
      .filter(attribute => ['integer', 'decimal', 'float'].includes(Grobeliskgroupreport._attributes[attribute].type));

    const searchBool = Object.keys(Grobeliskgroupreport._attributes)
      .filter(attribute => attribute !== Grobeliskgroupreport.primaryKey && !associations.includes(attribute))
      .filter(attribute => ['boolean'].includes(Grobeliskgroupreport._attributes[attribute].type));

    const query = (params._q || '').replace(/[^a-zA-Z0-9.-\s]+/g, '');

    return Grobeliskgroupreport.query(qb => {
      ...
      switch (Grobeliskgroupreport.client) {
        case 'pg': {
