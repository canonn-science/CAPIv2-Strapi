...
module.exports = {
  ...
  fetchAll: (params) => {
    ...
    return Apsite.query(function(qb) {
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Apsite.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Apsite._attributes)
      .filter(attribute => attribute !== Apsite.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Apsite._attributes[attribute].type));
