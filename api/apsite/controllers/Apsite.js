...
module.exports = {
  ...
  find: async (ctx, next, { populate } = {}) => {
    ctx.set('Content-Range', await Apsite.count());
    if (ctx.query._q) {
      return strapi.services.apsite.search(ctx.query);
    } else {
      return strapi.services.apsite.fetchAll(ctx.query, populate);
    }
  },
  ...
  count: async (ctx, next, { populate } = {}) => {
    return strapi.services.apsite.count(ctx.query, populate);
  },
