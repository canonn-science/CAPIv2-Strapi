...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const withRelated = populate || Fgsite.associations
      .filter(ast => ast.autoPopulate !== false)
      .map(ast => ast.alias);

    const filters = convertRestQueryParams(params);

    return Fgsite.query(buildQuery({ model: Fgsite, filters }))
  ...
  count: (params) => {
    ...
    return Fgsite.query(buildQuery({ model: Fgsite, filters: _.pick(filters, 'where') })).count();
  },
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = await Fgsite.forge(params).save(data);
