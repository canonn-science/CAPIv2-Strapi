...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const withRelated = populate || Grcodexdata.associations
      .filter(ast => ast.autoPopulate !== false)
      .map(ast => ast.alias);

    const filters = convertRestQueryParams(params);

    return Grcodexdata.query(buildQuery({ model: Grcodexdata, filters }))
  ...
  count: (params) => {
    ...
    return Grcodexdata.query(buildQuery({ model: Grcodexdata, filters: _.pick(filters, 'where') })).count();
  },
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = await Grcodexdata.forge(params).save(data);
