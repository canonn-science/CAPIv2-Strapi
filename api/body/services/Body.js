...
module.exports = {
  ...
  fetchAll: (params) => {
    // Get model hook
    const hook = strapi.hook[Body.orm];
    ...
    return Body.query(function(qb) {
      ...
      hook.load().generateMatchStage(qb)(Body, filters);
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Body.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Body._attributes)
      .filter(attribute => attribute !== Body.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Body._attributes[attribute].type));
