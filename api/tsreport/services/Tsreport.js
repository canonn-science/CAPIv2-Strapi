...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const withRelated = populate || Tsreport.associations
      .filter(ast => ast.autoPopulate !== false)
      .map(ast => ast.alias);

    const filters = convertRestQueryParams(params);

    return Tsreport.query(buildQuery({ model: Tsreport, filters }))
  ...
  count: (params) => {
    ...
    return Tsreport.query(buildQuery({ model: Tsreport, filters: _.pick(filters, 'where') })).count();
  },
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = await Tsreport.forge(params).save(data);
