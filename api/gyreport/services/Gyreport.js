...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const withRelated = populate || Gyreport.associations
      .filter(ast => ast.autoPopulate !== false)
      .map(ast => ast.alias);

    const filters = convertRestQueryParams(params);

    return Gyreport.query(buildQuery({ model: Gyreport, filters }))
  ...
  count: (params) => {
    ...
    return Gyreport.query(buildQuery({ model: Gyreport, filters: _.pick(filters, 'where') })).count();
  },
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = await Gyreport.forge(params).save(data);
