...
module.exports = {
  ...
  remove: async (params) => {
    ...
    await Bodysubtype.updateRelations(params);

    return Bodysubtype.forge(params).destroy();
  },

  /**
   * Promise to search a/an bodysubtype.
   *
   * @return {Promise}
   */
  ...
  search: async (params) => {
    ...
    const associations = Bodysubtype.associations.map(x => x.alias);
    const searchText = Object.keys(Bodysubtype._attributes)
      .filter(attribute => attribute !== Bodysubtype.primaryKey && !associations.includes(attribute))
      .filter(attribute => ['string', 'text'].includes(Bodysubtype._attributes[attribute].type));

    const searchNoText = Object.keys(Bodysubtype._attributes)
      .filter(attribute => attribute !== Bodysubtype.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Bodysubtype._attributes[attribute].type));

    const searchInt = Object.keys(Bodysubtype._attributes)
      .filter(attribute => attribute !== Bodysubtype.primaryKey && !associations.includes(attribute))
      .filter(attribute => ['integer', 'decimal', 'float'].includes(Bodysubtype._attributes[attribute].type));

    const searchBool = Object.keys(Bodysubtype._attributes)
      .filter(attribute => attribute !== Bodysubtype.primaryKey && !associations.includes(attribute))
      .filter(attribute => ['boolean'].includes(Bodysubtype._attributes[attribute].type));

    const query = (params._q || '').replace(/[^a-zA-Z0-9.-\s]+/g, '');

    return Bodysubtype.query(qb => {
