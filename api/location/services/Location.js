...
module.exports = {
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Location._attributes)
      .filter(attribute => attribute !== Location.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Location._attributes[attribute].type));
