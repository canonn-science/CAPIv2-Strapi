...
module.exports = {
  ...
  fetchAll: (params) => {
    ...
    return Cssite.query(function(qb) {
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Cssite.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Cssite._attributes)
      .filter(attribute => attribute !== Cssite.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Cssite._attributes[attribute].type));
