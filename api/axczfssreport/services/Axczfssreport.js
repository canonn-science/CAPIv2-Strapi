...
module.exports = {
  ...
  fetchAll: (params) => {
    ...
    return Axczfssreport.query(function(qb) {
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Axczfssreport.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Axczfssreport._attributes)
      .filter(attribute => attribute !== Axczfssreport.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Axczfssreport._attributes[attribute].type));
