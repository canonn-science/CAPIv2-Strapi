...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const withRelated = populate || Genlog.associations
      .filter(ast => ast.autoPopulate !== false)
      .map(ast => ast.alias);

    const filters = convertRestQueryParams(params);

    return Genlog.query(buildQuery({ model: Genlog, filters }))
  ...
  count: (params) => {
    ...
    return Genlog.query(buildQuery({ model: Genlog, filters: _.pick(filters, 'where') })).count();
  },
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = await Genlog.forge(params).save(data);
