...
module.exports = {
  ...
  find: async (ctx, next, { populate } = {}) => {
    ctx.set('Content-Range', await Hdreport.count());
    if (ctx.query._q) {
      return strapi.services.hdreport.search(ctx.query);
    } else {
      return strapi.services.hdreport.fetchAll(ctx.query, populate);
    }
  },
  ...
  count: async (ctx, next, { populate } = {}) => {
    return strapi.services.hdreport.count(ctx.query, populate);
  },
