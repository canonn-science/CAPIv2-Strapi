...
module.exports = {
  ...
  fetchAll: (params) => {
    // Get model hook
    const hook = strapi.hook[Hdreport.orm];
    ...
    return Hdreport.query(function(qb) {
      // Generate match stage.
      hook.load().generateMatchStage(qb)(Hdreport, filters);
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Hdreport.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Hdreport._attributes)
      .filter(attribute => attribute !== Hdreport.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Hdreport._attributes[attribute].type));
