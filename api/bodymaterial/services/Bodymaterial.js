...
module.exports = {
  ...
  remove: async (params) => {
    ...
    await Bodymaterial.updateRelations(params);

    return Bodymaterial.forge(params).destroy();
  },

  /**
   * Promise to search a/an bodymaterial.
   *
   * @return {Promise}
   */
  ...
  search: async (params) => {
    ...
    const associations = Bodymaterial.associations.map(x => x.alias);
    const searchText = Object.keys(Bodymaterial._attributes)
      .filter(attribute => attribute !== Bodymaterial.primaryKey && !associations.includes(attribute))
      .filter(attribute => ['string', 'text'].includes(Bodymaterial._attributes[attribute].type));

    const searchNoText = Object.keys(Bodymaterial._attributes)
      .filter(attribute => attribute !== Bodymaterial.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Bodymaterial._attributes[attribute].type));

    const searchInt = Object.keys(Bodymaterial._attributes)
      .filter(attribute => attribute !== Bodymaterial.primaryKey && !associations.includes(attribute))
      .filter(attribute => ['integer', 'decimal', 'float'].includes(Bodymaterial._attributes[attribute].type));

    const searchBool = Object.keys(Bodymaterial._attributes)
      .filter(attribute => attribute !== Bodymaterial.primaryKey && !associations.includes(attribute))
      .filter(attribute => ['boolean'].includes(Bodymaterial._attributes[attribute].type));

    const query = (params._q || '').replace(/[^a-zA-Z0-9.-\s]+/g, '');

    return Bodymaterial.query(qb => {
