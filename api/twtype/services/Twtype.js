...
module.exports = {
  ...
  fetchAll: (params) => {
    ...
    return Twtype.query(function(qb) {
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Twtype.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Twtype._attributes)
      .filter(attribute => attribute !== Twtype.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Twtype._attributes[attribute].type));
