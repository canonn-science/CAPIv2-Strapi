...
module.exports = {
  ...
  fetchAll: (params) => {
    // Get model hook
    const hook = strapi.hook[Gsobeliskgroup.orm];
    ...
    return Gsobeliskgroup.query(function(qb) {
      ...
      hook.load().generateMatchStage(qb)(Gsobeliskgroup, filters);
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Gsobeliskgroup.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Gsobeliskgroup._attributes)
      .filter(attribute => attribute !== Gsobeliskgroup.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Gsobeliskgroup._attributes[attribute].type));
