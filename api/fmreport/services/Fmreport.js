...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const withRelated = populate || Fmreport.associations
      .filter(ast => ast.autoPopulate !== false)
      .map(ast => ast.alias);

    const filters = convertRestQueryParams(params);

    return Fmreport.query(buildQuery({ model: Fmreport, filters }))
  ...
  count: (params) => {
    ...
    return Fmreport.query(buildQuery({ model: Fmreport, filters: _.pick(filters, 'where') })).count();
  },
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = await Fmreport.forge(params).save(data);
