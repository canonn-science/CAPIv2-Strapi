...
module.exports = {
  ...
  fetchAll: (params) => {
    // Get model hook
    const hook = strapi.hook[Gsreport.orm];
    ...
    return Gsreport.query(function(qb) {
      ...
      hook.load().generateMatchStage(qb)(Gsreport, filters);
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Gsreport.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Gsreport._attributes)
      .filter(attribute => attribute !== Gsreport.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Gsreport._attributes[attribute].type));
