...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const withRelated = populate || Tsstatus.associations
      .filter(ast => ast.autoPopulate !== false)
      .map(ast => ast.alias);

    const filters = convertRestQueryParams(params);

    return Tsstatus.query(buildQuery({ model: Tsstatus, filters }))
  ...
  count: (params) => {
    ...
    return Tsstatus.query(buildQuery({ model: Tsstatus, filters: _.pick(filters, 'where') })).count();
  },
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = await Tsstatus.forge(params).save(data);
