...
module.exports = {
  ...
  fetchAll: (params) => {
    // Get model hook
    const hook = strapi.hook[Gractiveobelisk.orm];
    ...
    return Gractiveobelisk.query(function(qb) {
      ...
      hook.load().generateMatchStage(qb)(Gractiveobelisk, filters);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Gractiveobelisk._attributes)
      .filter(attribute => attribute !== Gractiveobelisk.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Gractiveobelisk._attributes[attribute].type));
