...
module.exports = {
  ...
  fetchAll: (params) => {
    ...
    return Bttype.query(function(qb) {
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Bttype.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Bttype._attributes)
      .filter(attribute => attribute !== Bttype.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Bttype._attributes[attribute].type));
