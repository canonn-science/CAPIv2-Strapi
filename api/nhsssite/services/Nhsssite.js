...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const withRelated = populate || Nhsssite.associations
      .filter(ast => ast.autoPopulate !== false)
      .map(ast => ast.alias);

    const filters = convertRestQueryParams(params);

    return Nhsssite.query(buildQuery({ model: Nhsssite, filters }))
  ...
  count: (params) => {
    ...
    return Nhsssite.query(buildQuery({ model: Nhsssite, filters: _.pick(filters, 'where') })).count();
  },
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = await Nhsssite.forge(params).save(data);
