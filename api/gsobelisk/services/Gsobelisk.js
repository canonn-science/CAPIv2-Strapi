...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const withRelated = populate || Gsobelisk.associations
      .filter(ast => ast.autoPopulate !== false)
      .map(ast => ast.alias);

    const filters = convertRestQueryParams(params);

    return Gsobelisk.query(buildQuery({ model: Gsobelisk, filters }))
  ...
  count: (params) => {
    ...
    return Gsobelisk.query(buildQuery({ model: Gsobelisk, filters: _.pick(filters, 'where') })).count();
  },
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = await Gsobelisk.forge(params).save(data);
