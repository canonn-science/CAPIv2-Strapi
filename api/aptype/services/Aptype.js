...
module.exports = {
  ...
  fetchAll: (params) => {
    ...
    return Aptype.query(function(qb) {
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Aptype.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Aptype._attributes)
      .filter(attribute => attribute !== Aptype.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Aptype._attributes[attribute].type));
