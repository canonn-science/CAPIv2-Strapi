...
module.exports = {
  ...
  fetchAll: (params) => {
    ...
    return Excludecodex.query(function(qb) {
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Excludecodex.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Excludecodex._attributes)
      .filter(attribute => attribute !== Excludecodex.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Excludecodex._attributes[attribute].type));
