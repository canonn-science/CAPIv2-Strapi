...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const withRelated = populate || Gsartifact.associations
      .filter(ast => ast.autoPopulate !== false)
      .map(ast => ast.alias);

    const filters = convertRestQueryParams(params);

    return Gsartifact.query(buildQuery({ model: Gsartifact, filters }))
  ...
  count: (params) => {
    ...
    return Gsartifact.query(buildQuery({ model: Gsartifact, filters: _.pick(filters, 'where') })).count();
  },
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = await Gsartifact.forge(params).save(data);
