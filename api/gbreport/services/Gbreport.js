...
module.exports = {
  ...
  fetchAll: (params) => {
    // Get model hook
    const hook = strapi.hook[Gbreport.orm];
    ...
    return Gbreport.query(function(qb) {
      // Generate match stage.
      hook.load().generateMatchStage(qb)(Gbreport, filters);
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Gbreport.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Gbreport._attributes)
      .filter(attribute => attribute !== Gbreport.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Gbreport._attributes[attribute].type));
