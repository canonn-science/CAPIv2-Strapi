...
module.exports = {
  ...
  fetchAll: (params) => {
    // Get model hook
    const hook = strapi.hook[Tbreport.orm];
    ...
    return Tbreport.query(function(qb) {
      // Generate match stage.
      hook.load().generateMatchStage(qb)(Tbreport, filters);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Tbreport._attributes)
      .filter(attribute => attribute !== Tbreport.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Tbreport._attributes[attribute].type));
