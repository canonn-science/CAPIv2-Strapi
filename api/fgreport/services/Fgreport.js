...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const withRelated = populate || Fgreport.associations
      .filter(ast => ast.autoPopulate !== false)
      .map(ast => ast.alias);

    const filters = convertRestQueryParams(params);

    return Fgreport.query(buildQuery({ model: Fgreport, filters }))
  ...
  count: (params) => {
    ...
    return Fgreport.query(buildQuery({ model: Fgreport, filters: _.pick(filters, 'where') })).count();
  },
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = await Fgreport.forge(params).save(data);
