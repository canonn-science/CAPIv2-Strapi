...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const withRelated = populate || Gvreport.associations
      .filter(ast => ast.autoPopulate !== false)
      .map(ast => ast.alias);

    const filters = convertRestQueryParams(params);

    return Gvreport.query(buildQuery({ model: Gvreport, filters }))
  ...
  count: (params) => {
    ...
    return Gvreport.query(buildQuery({ model: Gvreport, filters: _.pick(filters, 'where') })).count();
  },
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = await Gvreport.forge(params).save(data);
