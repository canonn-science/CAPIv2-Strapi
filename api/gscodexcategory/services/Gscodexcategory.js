...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const withRelated = populate || Gscodexcategory.associations
      .filter(ast => ast.autoPopulate !== false)
      .map(ast => ast.alias);

    const filters = convertRestQueryParams(params);

    return Gscodexcategory.query(buildQuery({ model: Gscodexcategory, filters }))
  ...
  count: (params) => {
    ...
    return Gscodexcategory.query(buildQuery({ model: Gscodexcategory, filters: _.pick(filters, 'where') })).count();
  },
