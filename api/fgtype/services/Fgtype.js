...
module.exports = {
  ...
  fetchAll: (params) => {
    ...
    return Fgtype.query(function(qb) {
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Fgtype.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Fgtype._attributes)
      .filter(attribute => attribute !== Fgtype.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Fgtype._attributes[attribute].type));
