...
module.exports = {
  ...
  fetchAll: (params) => {
    // Get model hook
    const hook = strapi.hook[Grartifact.orm];
    ...
    return Grartifact.query(function(qb) {
      // Generate match stage.
      hook.load().generateMatchStage(qb)(Grartifact, filters);
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Grartifact.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Grartifact._attributes)
      .filter(attribute => attribute !== Grartifact.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Grartifact._attributes[attribute].type));
